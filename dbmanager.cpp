...
bool save_images(QString filename, int pageid)
{
    ...
    // If there are images, download them.
    if (image_png != false) {
...
{
    // check the html to see if there are any dependencies that needs to be downloaded
    if (!check_links(text)) {
        ...
        // checks if the files are already saved
...
void del_file(QString pageid)
{
    ...
    // checks if the folder exist , if exist delete it
    if (QDir(Folder_name).exists()) {
        dir = Folder_name;
        qDebug() << dir ;
...
// Checks if the page is updated. 
// If yes, then delete the old page and download the updated page.
